  ...
  val books: Set[Book] = Set(
    ...
    Book(title = "Programming in Scala",
         authors = List("Odersky, Martin", "Spoon, Lex", "Venners, Bill")))
                                                  //> books  : Set[week6.books.Book] = Set(Book(Effective Java 2,List(Bloch, Joshu
                                                  //| a)), Book(Programming in Scala,List(Odersky, Martin, Spoon, Lex, Venners, Bi
                                                  //| ll)), Book(Structure and Interpretation of Computer Programs,List(Abelson, H
                                                  //| arald, Sussman, Gerald J.)), Book(Effective Java,List(Bloch, Joshua)), Book(
                                                  //| Introduction to Functional Programming,List(Bird, Richard, Wadler, Phil)), B
                                                  //| ook(Java Puzzlers,List(Bloch, Joshua, Gafter, Neal)))
